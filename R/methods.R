...
#' @export
tau_weighted_bh <- function(Ps, Ws, tau=1, tau_pi0=0.5, Storey=FALSE){
  if (length(Ws) == 1){
    Ws <- rep(Ws, length(Ps))
  }
  if (Storey){
...
#' @export
weighted_storey_pi0 <- function(pvalues, weights=1, tau=0.5, m = length(pvalues)){
  w_max <- max(weights)
...
#' @export
sim_mtp <- function(pvalues, pvalues2, alpha, grid_size = 40, details=FALSE, Storey=FALSE){
  is_na_secondary_idx <- is.na(pvalues2)

  comb_grid <- seq(from = 0, to = pi/2, length.out = grid_size)
  z1 <- stats::qnorm(pvalues)
  z2 <- stats::qnorm(pvalues2)
  rjs <- rep(NA, grid_size)
  for (i in 1:grid_size){
    theta <- comb_grid[i]
    combined_pv <- stats::pnorm(cos(theta)*z1 + sin(theta)*z2)
    combined_pv[is_na_secondary_idx] <- pvalues[is_na_secondary_idx]
    rjs[i] <- sum(stats::p.adjust(combined_pv, method="BH") <= alpha)
  }
  idx_max <- which.max(rjs)

  theta_opt <- comb_grid[idx_max]
  p <- stats::pnorm(cos(theta_opt)*z1 + sin(theta_opt)*z2)
  p[is_na_secondary_idx] <- pvalues[is_na_secondary_idx]

  adj_p <- tau_weighted_bh(p, 1, tau=0.5, Storey=Storey)

  if (!details){
    return(adj_p <= alpha)
  } else {
    return( list(total_rjs = rjs[idx_max],
                 theta_opt = theta_opt,
                 rjs = rjs,
                 Storey=Storey,
                 p = p,
